...
def next_gen(params):
    ...
    with open('quick_analysis_data_GA7.csv', 'a') as quick_file:
    ...
    for x in range(len(fitness_list[0])):
            ...
            with open('full_analysis_data_GA7.csv', 'a') as analysis_file:
...
def init_gen(pop_size, unit_list):
    ...
    setup = subprocess.call('mkdir predictions_GA7', shell=True)

    # create new analysis files
    with open('quick_analysis_data_GA7.csv', mode='w+') as quick:
    ...
    with open('full_analysis_data_GA7.csv', mode='w+') as full:
