...
def PCE_prediction(population):
    ...
    # load the model
    model = load(open('gb_mfcounts_standardized_above10.pkl', 'rb'))
    # load the scaler
    ...
    for x in range(len(population)):
        ...
        exists = os.path.isfile('PCE_predictions_gb/%s.txt' % (filename))
        if exists:
            with open('PCE_predictions_gb/%s.txt' % (filename), 'r') as file:
            ...
            for i in range(len(donors)):
                ...
                with open('PCE_predictions_gb/%s.txt' % (filename), 'w') as file:
