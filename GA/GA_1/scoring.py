...
def donor_to_csv(filename, donor):
    with open(filename, 'r', encoding = 'utf-8') as file:
        line = file.readline()
        oscs = []
        energyEV = []
        ...
        df_donor['eV'] = don_spectraEV
        df_donor['nm'] = don_spectraNM
        df_donor['intensity'] = don_spectraIntensity

        filename_abs = 'donor_abs_csvs/' + donor + '_abs.csv'
        df_donor.to_csv(filename_abs)

def overlap(acc_spectraIntensity, donor):
    
    donor = pd.read_csv('../../donor_abs_csvs/' + donor + '_abs.csv')
    
    don_spectraIntensity = donor['intensity']
    don_spectraNM = donor['nm']
    
    overlapped_spectra_intensities = [don_spectraIntensity[i] * acc_spectraIntensity[i] for i in range(len(don_spectraIntensity))]
    
    area_altered_spectra = np.trapz(np.flip(overlapped_spectra_intensities), np.flip(don_spectraNM), dx=0.1, axis=- 1)    
...
def PCE_prediction(population):
    ...
    donors = pd.read_csv('../../donor_properties_fixed.csv')
    ...
    for x in range(len(population)):
        ...
        exists = os.path.isfile('PCE_predictions_ensemble_rf_ann/%s.txt' % (filename))
        if exists:
            with open('PCE_predictions_ensemble_rf_ann/%s.txt' % (filename), 'r') as file:
                content = file.readline()
                content_list = content.split()
                best_PCE = float(content_list[1])
                best_donor = content_list[2]

        else:
            try:
                acc_stda = '../sTDDFTxtb_output/' + filename + '.stda'
                acc_mol = '../GFN2_output/' + filename + '.mol'
                acc_GFN2 = '../GFN2_output/' + filename + '.out'
                acc_solv_water = '../solvation_water/' + filename + '.out'
                acc_solv_hexane = '../solvation_hexane/' + filename + '.out'

                # parse sTDDFT-xtb output files of acceptors
                sTDDFTxtb_output, acc_spectraEV, acc_spectraNM, acc_spectraIntensity = parse_sTDA(acc_stda) #HOMO, HOMOminus1, LUMO, LUMOplus1, fundbg, deltaHOMO, deltaLUMO, opt_bg, max_abs, summed_oscs, area_spectra, area_sim_solar_spectra, chemical_potential, electrophilicity
                # parse GFN2-xtb files for acceptor
                pi_size = pi_sys_size(acc_mol) #pi_size
                rdkit_desc = rdkit_descriptors(acc_mol) #num_rot_bonds, MolLogP, TPSA, NumHAcceptors, NumHDonors
                plan = planarity(acc_mol) # planarity

                # calculate pi system size of acceptor
                GFN2 = parse_GFN2(acc_GFN2) #dipole_moment, polarizability

                # calculate solvation free energy of acceptor in water
                solv_water = solvation(acc_solv_water) #solvation_energy

                # calculate solvation free energy of acceptor in hexane
                solv_hexane = solvation(acc_solv_hexane) #solvation_energy

                # For morgan fingerprint counts
                acc_fp = morgan_fp_counts(acc_mol)
                
                # acceptor absorption spectra
                #acc_spectraEV, acc_spectraNM, acc_spectraIntensity = acc_abs_spectra(acc_stda)
                ...
                for i in range(len(donors)):
                    ...
                    donor_props = list(donors.iloc[i])[2:28]

                    overlap_AD = overlap(acc_spectraIntensity, list(donors.iloc[i])[1])

                    AD_HOMOoffset = donor_props[0] - sTDDFTxtb_output[0]
                    AD_LUMOoffset = donor_props[2] - sTDDFTxtb_output[2]
                    DHOMO_ALUMO_offset = sTDDFTxtb_output[2] - donor_props[0]
                    ...
                    fp.extend((list(donors.iloc[i])[28:]))
                    ...
                    if PCE > best_PCE:
                        best_PCE = PCE
                        best_donor = list(donors.iloc[i])[1]
            ...
            with open('PCE_predictions_ensemble_rf_ann/%s.txt' % (filename), 'w') as file:
                        file.write('%s %s %s' %(filename, best_PCE, best_donor))


        PCE_list.append(best_PCE)
        NFA_name.append(filename)
        best_donors.append(best_donor)
    ...
    for x in ranked_indices:
        ...
        ranked_best_donor.append(best_donors[x])
        ranked_NFA_population.append(NFA_population[x])

    ranked_population = [ranked_NFA_names, ranked_PCE, ranked_best_donor, ranked_NFA_population]
