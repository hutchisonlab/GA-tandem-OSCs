...
def PCE_prediction(population):
    ...
    # load the model
    model = load(open('../rf_mfcounts_standardized_above10.pkl', 'rb'))
    # load the scaler
    ...
    for x in range(len(population)):
        ...
        exists = os.path.isfile('PCE_predictions/%s.txt' % (filename))
        if exists:
            with open('PCE_predictions/%s.txt' % (filename), 'r') as file:
            ...
            for i in range(len(donors)):
                ...
                with open('PCE_predictions/%s.txt' % (filename), 'w') as file:
