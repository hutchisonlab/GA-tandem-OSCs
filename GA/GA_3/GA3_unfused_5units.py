...
def next_gen(params):
    ...
    with open('quick_analysis_data_GA3.csv', 'a') as quick_file:
    ...
    for x in range(len(fitness_list[0])):
        ...
        with open('full_analysis_data_GA3.csv', 'a') as analysis_file:
    ...
    # mutate units. Can select either left terminal, left spacer, core, right spacer, or right terminal
        ...
        if current_sequence == 0:
            ...
            # mutate core
                ...
                core = random.randint(0, len(unit_list[3]) - 1)
...
def init_gen(pop_size, unit_list):
    ...
    setup = subprocess.call('mkdir PCE_predictions_GA3 FF_optimized GFN2_output sTDDFTxtb_output solvation_hexane solvation_water', shell=True)
  
    # run GFN2-xTB, sTD-DFT-xtb, and xtb solvation in hexane and water
    run_calculations(population_str, population)

    # create new analysis files
    with open('quick_analysis_data_GA3.csv', mode='w+') as quick:
        quick_writer = csv.writer(quick)
        quick_writer.writerow(['gen', 'max_PCE', 'min_PCE', 'med_PCE'])

    with open('full_analysis_data_GA3.csv', mode='w+') as full:
