...
def next_gen(params):
    ...
    fitness_list = scoring_smear08.fitness_prediction(new_population, unit_list) #[ranked_pairs, ranked_cmbined_abs_values, ranked_overlap_values, ranked_scores]

    max_abs = fitness_list[1][-1]
    min_abs = fitness_list[1][int((len(fitness_list[1])-1)/2)]
    med_abs = fitness_list[1][0]

    min_overlap = fitness_list[2][-1]
    max_overlap = fitness_list[2][int((len(fitness_list[2])-1)/2)]
    med_overlap = fitness_list[2][0]
    ...
    with open('quick_analysis_data_GA5_smear08_normalized.csv', 'a') as quick_file:
    ...
    for x in range(len(fitness_list[0])):
            ...
            with open('full_analysis_data_GA5_smear08_normalized.csv', 'a') as analysis_file:
...
def init_gen(pop_size, unit_list):
    ...
    setup = subprocess.call('mkdir predictions_GA5_smear08_normalized', shell=True)

    # create new analysis files
    with open('quick_analysis_data_GA5_smear08_normalized.csv', mode='w+') as quick:
    ...
    with open('full_analysis_data_GA5_smear08_normalized.csv', mode='w+') as full:
        full_writer = csv.writer(full)
        full_writer.writerow(['gen', 'pair', 'combined_abs', 'overlap', 'score'])

    fitness_list = scoring_smear08.fitness_prediction(population, unit_list) #[ranked_pairs, ranked_cmbined_abs_values, ranked_overlap_values, ranked_scores]
